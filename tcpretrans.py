...
if args.port:
  ...
  port_filter = 'if (%s) { return 0; }' % (dports_if) 
  
bpf_text = bpf_text.replace('##FILTER_PORT##', port_filter)
...
def print_ipv4_event(cpu, data, size):
    ...
    print("%-8s ~ %-6d ~ TCPv%-2d ~ %-16s ~ %-6s ~ %1s> ~ %-16s ~ %-6s ~ %s ~" % ( # changed
        strftime("%H:%M:%S"), event.pid, event.ip, inet_ntop(AF_INET, pack('I', event.saddr)), event.lport,
        type[event.type], inet_ntop(AF_INET, pack('I', event.daddr)), event.dport,
        tcpstate[event.state]))

def print_ipv6_event(cpu, data, size):
    event = b["ipv6_events"].event(data)
    print("%-8s ~ %-6d ~ TCPv%-2d ~ %-16s ~ %-6s ~ %1s> ~ %-16s ~ %-6s ~ %s ~" % ( # changed 
        strftime("%H:%M:%S"), event.pid, event.ip, inet_ntop(AF_INET6, event.saddr), event.lport,
        type[event.type], inet_ntop(AF_INET6, event.daddr), event.dport,
        tcpstate[event.state]))
...
# read events
    ...
    # header
    # changed
    print("%-8s %-6s %-6s %-16s %-6s %1s %-16s %-6s %-4s" % ("TIME", "PID", "PORT",
