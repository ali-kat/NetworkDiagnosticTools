...
# Modified by: Alexandra Pattillo
from __future__ import print_function
from bcc import BPF

import argparse as ap
from socket import inet_ntop, ntohs, AF_INET, AF_INET6 # changed 
...
parser.add_argument("-P", "--port", # added 
                    help="trace all ports but the specified comma separated ports.")
...
int trace_connect_v4_return(struct pt_regs *ctx) // LOOK_AT
{
...
int trace_connect_v6_return(struct pt_regs *ctx) // LOOK_AT
{
  int ret = PT_REGS_RC(ctx);
  u64 pid = bpf_get_current_pid_tgid();
...
int trace_close_entry(struct pt_regs *ctx, struct sock *skp) // LOOK_AT
{
#if CGROUPSET
  u64 cgroupid = bpf_get_current_cgroup_id();
  if (cgroupset.lookup(&cgroupid) == NULL) {
...
int trace_accept_return(struct pt_regs *ctx) // LOOK_AT 
{
#if CGROUPSET
  u64 cgroupid = bpf_get_current_cgroup_id();
  if (cgroupset.lookup(&cgroupid) == NULL) {
  ...
  // pull in details
  u16 lport = 0, sport = 0, dport = 0;
  u32 net_ns_inum = 0;
  u8 ipver = 0;

  struct inet_sock *sockp = (struct inet_sock *) newsk;
  sport = sockp->inet_sport;
...
def print_ipv4_event(cpu, data, size):
    ...
    if args.timestamp:
      print("%-8s ~" % strftime("%H:%M:%S"), end="") # changed 
    if event.type == 1:
        type_str = "C"
    elif event.type == 2:
        type_str = "A"
    ...
    print("%-6d ~ %-16s ~ TCPv%-2d ~ %-16s ~ %-16s ~ %-6d ~ %-6d ~" % #changed
...
if args.verbose:
    if args.timestamp:
        print("%-14s" % ("TIME"), end="")
    print("%-12s %-6s %-16s %-6s %-16s %-16s %-6s %-7s" % ("TYPE",
          "PID", "COMM", "PORT", "SADDR", "DADDR", "SPORT", "DPORT"), end="")
    if not args.netns:
        print("%-8s" % "NETNS", end="")
    print()
else:
    if args.timestamp:
        print("%-9s" % ("TIME"), end="")
    print("%-2s %-6s %-16s %-6s %-16s %-16s %-6s %-6s" %
          ("T", "PID", "COMM", "PORT", "SADDR", "DADDR", "SPORT", "DPORT"))
