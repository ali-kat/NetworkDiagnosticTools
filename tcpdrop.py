...
int trace_tcp_drop(struct pt_regs *ctx, struct sock *sk, struct sk_buff *skb)
{
    ...
    ##FILTER_PORT##
    // WHY DO THE PORTS HAVE TO BE FILTERED HERE
...
if args.port:
  ...
  dport_if = ' && '.join(['dport == %d' % ntohs(port) for port in ports])
  sport_if =  ' && '.join(['sport == %d' % ntohs(port) for port in ports]) 
  port_filter = 'if (%s || %s) { return 0; }' % (dport_if, sport_if) 
...
def print_ipv4_event(cpu, data, size):
    ...
    print("%-8s ~ %-6d ~ TCPv%-2d ~ %-16s ~ %-6s ~ %-16s ~ %-6s ~ %s ~ (%s) ~" % ( # changed
        strftime("%H:%M:%S"), event.pid, event.ip, inet_ntop(AF_INET, pack('I', event.saddr)), event.sport,
...
# header
# changed
print("%-8s %-6s %-6s  %-16s %-6s >  %-16s %-6s %s (%s)" % ("TIME", "PID", "PORT",
