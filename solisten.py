...
import os
from socket import inet_ntop, ntohs, AF_INET, AF_INET6, SOCK_STREAM, SOCK_DGRAM # changed 
...
def event_printer(show_netns):
    def print_event(cpu, data, size):
        ...
        # Display
        if show_netns:
            printb(b"%-8s ~ %-6d ~ %-20.20s ~ %-12d ~ %-6s ~ %-8d ~ %-5d ~ %-39s ~" % ( # changed
                strftime("%H:%M:%S").encode(), pid, event.task, event.netns, protocol.encode(), event.backlog,
                event.lport, address.encode(),
            ))
        else:
            printb(b"%-8s ~ %-6d ~ %-20.20s ~ %-6s ~ %-8d ~ %-5d ~ %-39s ~" % ( #changed 
                strftime("%H:%M:%S").encode(), pid, event.task, protocol.encode(), event.backlog,
                event.lport, address.encode(),
            ))
...
if __name__ == "__main__":
    ...
    # Print headers
    if args.show_netns:
        print("%-8s %-6s %-12s %-12s %-6s %-8s %-5s %-39s" % # changed too
              ("TIME", "PID", "COMM", "NETNS", "PROTO", "BACKLOG", "LPORT", "LADDR"))
    else:
        print("%-8s %-6s %-12s %-6s %-8s %-5s %-39s" %
              ("TIME", "PID", "COMM", "PROTO", "BACKLOG", "LPORT", "LADDR"))
